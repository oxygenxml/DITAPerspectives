...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <graph description="DITA Topic Stats">
            <nodes>
                <node name="elements" label = "DITA topic elements | {$totalElementsNumber}">
                    <node name="baseTopicElements" label="base elements | {$baseTopicElementsNumber}">
                        <xsl:for-each select="$baseTopicElements">
                            <node name="{@name}"/>
                        </xsl:for-each>
                    </node>
                    <node name="otherElements" label="specialized elements | {$otherTopicElementsNumber}"/>
                </node>
            </nodes>
        </graph>
    </xsl:template>
